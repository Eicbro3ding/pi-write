...
- **交互控件收敛成唯一实现**:下拉(`Select.tsx` + `select-logic.ts`)、主题卡(`ThemeCards.tsx`)、长内容折叠(`fold.ts` + `FoldablePre.tsx`)、工具动作流(`tool-status.ts`)都是「一个实现 + 纯逻辑单测」,页面只负责摆位置。原生 `<select>` 被替换是因为它的展开层**不可样式化**(圆角 / 阴影 / hover 高亮由系统画,macOS 上整行 CSS 无效)——这类「控制不了的部分」在暗色主题里会直接破相。
...
- **简化输出 ≠ 什么都不显示**:工具调用压成一行带宾语的动作(在改哪个文件)、完成的行留在流水里、用线性图标而非 emoji(各平台渲染不一致、颜色不可控);唯一例外是 `bash`——它是唯一能越过书目录边界的工具,永远给完整卡片。
