...
// nodejs-mobile(Node 18,Android)无 ICU:Intl 整体未定义(实测 ReferenceError)。
// 回退为按码点切分的极简 segmenter(仅 .segment() 迭代 {segment},pi-tui 只用这个面)。
function fallbackSegmenter(): Intl.Segmenter {
	return {
		segment(text: string): IterableIterator<{ segment: string }> {
			return (function* () {
				for (const ch of text) yield { segment: ch };
			})();
		},
...
const graphemeSegmenter = hasIntlSegmenter
	? new Intl.Segmenter(undefined, { granularity: "grapheme" })
	: fallbackSegmenter();
const wordSegmenter = hasIntlSegmenter
...
// Node18 兼容:全部由 /v(Unicode Sets,Node 20+)改为 /u;属性类 \p{...} 在 /u 下可用,语义不变。
...
// Node18 兼容:`--`(字符差集)是 /v 专属语法,用负向前瞻模拟等价语义。
const terminalSpacingMarkRegex =
	/^(?:(?![\u1734\u302E\u302F])[\p{Spacing_Mark}]|[\u065F\u0F7F\u102B\u102C\u1031\u1033-\u1035\u1038\u103A-\u103E])+$/u;
// Node18 兼容:改用 \p{Extended_Pictographic}(/u 可用)近似 RGI_Emoji;
// 差异仅在 ZWJ 序列与个别码点,影响终端列宽显示,无功能影响。
